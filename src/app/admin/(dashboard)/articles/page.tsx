...
export default function AdminArticlesPage() {
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <header className="pb-6 border-b border-stone-gray/40 dark:border-slate-700/40 shrink-0 flex items-center justify-between">
        <div>
          <h2 className="font-headline-md text-2xl text-legal-navy dark:text-slate-100 font-bold">Makaleleri Yönet</h2>
                ...
                articles.map((art) => (
                  <tr key={art.id} className="hover:bg-stone-50 dark:hover:bg-slate-700/50 dark:bg-slate-900/50 transition-colors">
                    <td className="px-6 py-4 max-w-sm">
                      <div className="font-bold text-legal-navy dark:text-slate-100 text-sm truncate">{art.title}</div>
                      <div className="text-xs text-on-surface-variant dark:text-slate-400 truncate mt-0.5 font-medium">
                        {art.summary}
                      </div>
                    </td>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {art.categories && art.categories.length > 0 ? (
                          art.categories.map((c) => (
                            <span
                              key={c.id}
                              className="bg-stone-100 dark:bg-slate-700 px-2 py-0.5 border border-stone-200 rounded text-[9px] font-bold text-legal-navy dark:text-slate-100"
                            >
                              {c.name}
                            </span>
                          ))
                        ) : (
                          <span className="bg-stone-100 dark:bg-slate-700 px-2 py-0.5 border border-stone-200 rounded text-[9px] font-bold text-legal-navy dark:text-slate-100">
                            Genel
                          </span>
                        )}
                      </div>
                    ...
                    <td className="px-6 py-4 text-xs text-on-surface-variant dark:text-slate-400 font-bold">
                      {art.view_count || 0}
                    </td>
                    <td className="px-6 py-4">
          ...
          <div className="relative bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-stone-gray dark:border-slate-700 max-w-sm w-full mx-4 overflow-hidden z-10">
            <div className="p-6">
              <div className="flex items-start gap-4 mb-6">
                ...
                <div>
                  <h4 className="font-bold text-legal-navy dark:text-slate-100 text-sm">Makaleyi Sil</h4>
